...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 title: false,
			 pagestat: 'Menampilkan {from} s/d {to} dari {total} item',
			 pagetext: 'Hal',
			 outof: 'dari',
			 findtext: 'Cari',
			 procmsg: 'Sedang proses, silakan menunggu ...',
			 query: '',
			 qtype: '',
			 nomsg: 'Tidak ada item',
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>"+p.findtext+" <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='Hapus Pencarian' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[value=Hapus Pencarian]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
