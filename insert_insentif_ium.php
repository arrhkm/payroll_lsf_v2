...
$sql_ium="SELECT * FROM insentif_uang_makan WHERE emp_id='$_REQUEST[emp_id]' AND kd_periode='$_REQUEST[kd_periode]'";
...
<div id="templatemo_main" class="wrapper">
	...
	<h2>Insert insentif Uang Makan <?php echo $row_emp[emp_id].", ".$row_emp[emp_name];?></h2>     
		...
		<tr>
		<td>
		<form name="form1" method="POST" action="tambah_insentif_ium.php">		
		<?php
			   ...
			   while($row=mysqli_fetch_assoc($rs_absen))
				...
				<tr>
				<td><?php echo $no;?>
				<input type=checkbox name=cek[] value="<?=$row['tgl']?>" id=<?php echo "id-".$no;?>> 
				</td>
				<td><?php echo date("l",strtotime($row['tgl'])).", ".$row['tgl'].", ".$row['jam_in']."-".$row['jam_out']; if ($row['jam_out']>24) echo ".:";?></td>
				</tr></table> <?php
				$no++;
				}	
				?>
				...
				<input type='button' value='Back' name='btn_back' class='bordered' onClick="location='emp_insentif_ium.php?kd_periode=<?php echo $_REQUEST['kd_periode'];?>'">
		</form>
		</td>
		</tr>
		</table>
		...
		<?php while ($row_ium=mysqli_fetch_assoc($rs_ium)) { ?>
		<tr>
		<td><?=$row_ium['kd_ium']?></td>
		<td><?=$row_ium['kd_periode']?></td>
		<td><?=$row_ium['tgl_ium']?></td>
		<td><?=$row_ium['jml_ium']?></td>
		<td><a href="saveinsert_insentif_ium.php?delete=1&kd_ium=<?php echo $row_ium[kd_ium];?>&emp_id=<?php echo $_REQUEST[emp_id];?>&kd_periode=<?php echo $_REQUEST[kd_periode];?>">Delete</a></td>
		</tr>
		<?php } ?>
		</table>
	<div class="clear"></div>
