...
$rs_project = $db->query($qry_project) or die(mysql_error());
$SQL_periode = "SELECT * FROM periode ORDER BY tgl_awal DESC LIMIT 0,20";
$rs_periode = $db->query($SQL_periode) or die(mysql_error());
...
<div id="templatemo_main" class="wrapper">
	<!-- Tempat Menaruh Tabel ISI -->
		...
		<?php while ($row_project=$db->fetch_array($rs_project)) { ?>
		...
		<?php while ($row_periode=$db->fetch_array($rs_periode)) { ?>
