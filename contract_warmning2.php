...
c. start_kontrak, c.end_kontrak, c.lama_kontrak, timestampdiff(DAY, end_kontrak, curdate()) as lamax
FROM employee a, jabatan b, kontrak_kerja c
WHERE 
c.emp_id=a.emp_id AND 
b.kd_jabatan= a.kd_jabatan
 AND timestampdiff(DAY, end_kontrak, curdate()) <14
 AND timestampdiff(DAY, end_kontrak, curdate()) >0

";
$RSwarm=$db->query($SQLwarm);
...
<!-- div class="searchbox" -->
	<!-- Tempat Menaruh Tabel ISI -->
		...
		<th filter='true'>Kontrak ke-</th>
		<th filter='true'>Start</th>
		<th filter='true'>End</th>		
		<th  filter='false' width="50px" colspan ="" align=center></th>		
		</tr>
		</thead>
		<?php
		if ($db->num_rows($RSwarm) >0 ){
		while($ROWwarm = $db->fetch_array($RSwarm)) { ?>
		<tr align="center">
		<td align="rigt"><?php echo $ROWwarm[emp_id];?></td>
		<td align="left"><?php echo $ROWwarm[emp_name];?></td>
		<td align="left"><?php echo $ROWwarm[nama_jabatan];?></td>
		<td align="left"><?php echo $ROWwarm[no_kontrak];?></td>
		<td align="left"><?php echo $ROWwarm[start_kontrak];?></td>
		<td align="left"><?php echo $ROWwarm[end_kontrak];?></td>	
		<td width="50px">
		<a href="#.php?&emp_id=<?php //echo $ROWwarm[emp_id];?>">Add </a>		
		</td>		
		</tr>
